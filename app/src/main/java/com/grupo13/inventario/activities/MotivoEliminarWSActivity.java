...
public class MotivoEliminarWSActivity extends AppCompatActivity {

    @BindView(R.id.edtIDEliminarMotivo)
    EditText edtIDEliminarMotivo;

    private final String url = "https://eisi.fia.ues.edu.sv/eisi13/inventariows/motivo/eliminar.php";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_motivo_eliminar_w_s);
        ButterKnife.bind(this);
        StrictMode.ThreadPolicy policy = new StrictMode.ThreadPolicy.Builder().permitAll().build();
        StrictMode.setThreadPolicy(policy);
    }
    public void eliminarMotivoWS(View v){
        String mensaje = "";
        String motivo_id = edtIDEliminarMotivo.getText().toString();
        if(!motivo_id.isEmpty()) {
            try {
                JSONObject elementoEliminar = new JSONObject();
                elementoEliminar.put("motivo_id", motivo_id);
